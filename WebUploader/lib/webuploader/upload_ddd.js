(function ($) {
    ...
    $(function () {
        ...
        // 没选择文件之前的内容。
            ...
            //$progress = $('.progress').hide(),// $statusBar.find('.progress').hide(),
                ...
                .then(function (val) {
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            ...
                            Global.FileQueueds.push({ id: file.id, md5: val, size: file.size, ext: file.ext, chunk: data.chunk, savePath: decodeURIComponent(data.savePath), virtualPath: decodeURIComponent(data.virtualPath) });
                            console.info('fileCheckMaxChunk', file, res.data);
                            fileObj.find('.progress_check').attr('data-checkedcomplete', true).text('验证完成，等待上传').css('color', '#aaa');
                            $upload.css("display", "");
                            $("#filePicker").css("display", "none");
                            fmd5 = val;
                            fpath = decodeURIComponent(data.virtualPath);
                            ////文件验证完成后自动触发上传
                            //uploader.upload();
                        }
                    });
        ...
        function addFile(file) {
            var $li = $('<li id="' + file.id + '">' +
                    ...
                    '</li>'),
                $btns = $('<div class="file-panel"><span class="cancel" title="删除">删除</span></div>').appendTo($li),
            ...
            $li.appendTo($queue);
            $("#uploader .delete").attr("style","margin-left:5px; cursor:pointer;");
        }
        ...
        $("#uploader .delete").click(function () {
            ...
            $("#uploader .delete").attr("style", "display:none;");
        });

        //下载文件
        $("#uploader .download").click(function () {
            var url = window.location.protocol + "//" + window.location.host + fpath;
            $(this).attr("href", url);
        });
        ...
        uploader.on('uploadSuccess', function (file, response) {
            ...
            if (response && response.code >= 0) {
                ...
                var savePath = decodeURIComponent(dataObj.savePath);
                var virtualPath = decodeURIComponent(dataObj.virtualPath);
                fpath = decodeURIComponent(virtualPath);
